...
public class Fractale {
	...
	private IntegerProperty ZOOM = new SimpleIntegerProperty(200);
	double borneX1;
	double borneX2;
	double borneY1;
	double borneY2;
	...
	public Fractale(){
		...
		ZOOM.addListener(new ChangeListener<Number>(){
			...
			public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
				System.out.println("zoom change");
	...
	private void calcul(){
		...
		largeur=(borneX2-borneX1)*ZOOM.getValue();
		hauteur=(borneY2-borneY1)*ZOOM.getValue();
		...
		for(int x=0;x<largeur;x++){
			...
			for(int y=0;y<hauteur;y++){             
				...
				c_r=(double)x/ZOOM.getValue() + borneX1;
				c_i=(double)y/ZOOM.getValue() + borneY1;
